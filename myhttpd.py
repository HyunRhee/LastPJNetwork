...
def dobody(cgiProg, qString):
    prog = './login.cgi.py'
    try:
        proc = subprocess.Popen([prog], stdin=envCGI, stdout=subprocess.PIPE)
        code = '200'
...
signal.signal(signal.SIGINT, shutdownServer) #발생이벤트 종료
signal.signal(signal.SIGCHLD, collectZombie) #자식프로세스 종료
